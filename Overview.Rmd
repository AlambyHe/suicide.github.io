...
## Is susicide a worldwide public heahth problem?
...
```{rinclude=F}
age_color = as.factor(colnames(continent_age_tibble)) 

# use plotly to create histogram
plot_ly(continent_age_tibble, x = ~continent, y = ~x5_14_years, type = "bar", marker = list(color = "#81C7D4")) %>% 
  add_trace(y = ~x15_24_years, marker = list(color = "#33A6B8")) %>% 
  add_trace(y = ~x25_34_years, marker = list(color = "#0089A7")) %>% 
  add_trace(y = ~x35_54_years, marker = list(color = "#336774")) %>% 
  add_trace(y = ~x55_74_years, marker = list(color = "#255359")) %>% 
  add_trace(y = ~x75_years, marker = list(color = "#0C4842"))
```


### We should pay more attention to suicide.
